import { reverseTheNumber } from "../utils/reverseTheNumber";
...
// таких как 196, невозможно получить палиндром.Такое число,
// которое не образует палиндром путем переворачивания и сложения с самим собой,
...
// пока не будет доказано обратное.Помимо этого дано,
// что любое число меньше десяти тысяч либо(1) станет палиндромом меньше,
// чем за 50 итераций, либо(2) никто, с какой бы - то ни было вычислительной мощностью,
// не смог получить из него палиндром.Между прочим, 10677 является первым числом,
// для которого необходимо более 50 итераций,
...
let countLincherelNumbers = 0;

const isLincherelNumber = (number: number): boolean => {
  for (let i = 0; i < 50; i++) {
    const reversed = reverseTheNumber(number);
    const sum = number + reversed;

    if (sum === reverseTheNumber(sum)) {
      return false;
    }
    number = sum;
  }
...
for (let i = 1; i < MAX_LIMIT; i++) {
  let number = i;
  if (isLincherelNumber(number)) {
    countLincherelNumbers++;
  }
}

console.log(countLincherelNumbers);
